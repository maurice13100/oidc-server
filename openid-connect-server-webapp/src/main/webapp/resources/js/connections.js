 ...
 *******************************************************************************/
var UserConnectionModel = Backbone.Model.extend({
	idAttribute: 'id',
	
	initialize: function() { },
	
	urlRoot: 'api/userconnections'
	
});

var UserConnectionCollection = Backbone.Collection.extend({
  ...
  ,
	...
	url: function() {
        return 'api/userconnections/' + this.id;
    }

});


var UserConnectionListView = Backbone.View.extend({
	tagName: 'span',
	...
	render:function (eventName) {
		$(this.el).html($('#tmpl-uconnection-table').html());
		
		var approvedSiteCount = 0;
		
		var _self = this;
		
		_.each(this.model.models, function(userConnection) {
			// look up client
			//var client = this.options.clientList.getByClientId(userConnection.get('clientId'));
			var view = new UserConnectionView({model: userConnection}); ///, client: client});
			view.parentView = _self;
			$('#uconnection-table', this.el).append(view.render().el);
			//if (client != null) {
...
var UserConnectionView = Backbone.View.extend({
	tagName: 'tr',
	
	initialize: function(options) {
    	this.options = options;
		if (!this.template) {
			this.template = _.template($('#tmpl-uconnection').html());
		}
	},

    render: function() {

		var json = { username : this.model.get("username") , date : this.model.get("date"), sub : this.model.get("sub"),  phoneNumber : this.model.get("phoneNumber")};
		...
		//var json = { userId : this.model.get("userId") , formattedDate : formattedDate};
