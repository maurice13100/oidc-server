...
INSERT INTO user_info_TEMP (sub, preferred_username, name, email, email_verified, phone_number, phone_number_verified) VALUES
  ('90342.ASDFJWFA','admin','Demo Admin','admin@example.com', false, '+3317514306', true),
  ('01921.FLANRJQW','user','Demo User','user@example.com', true, '+3317514306', true);
...
MERGE INTO user_info 
  USING (SELECT sub, preferred_username, name, email, email_verified, phone_number, phone_number_verified FROM user_info_TEMP) AS vals(sub, preferred_username, name, email, email_verified, phone_number, phone_number_verified)
  ON vals.preferred_username = user_info.preferred_username
  WHEN NOT MATCHED THEN 
    INSERT (sub, preferred_username, name, email, email_verified, phone_number, phone_number_verified) VALUES (vals.sub, vals.preferred_username, vals.name, vals.email, vals.email_verified, vals.phone_number, vals.phone_number_verified);
